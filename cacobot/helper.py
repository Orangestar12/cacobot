...
async def away(message, client):
    ...
    *Example: `.away I'm busy fending off this marine dude. DM me if you need me! `❤` `*
    ...
    if len(message.content.split()) == 1:
        await client.send_message(message.channel, '{}: Please provide a message that I will respond with when you are mentioned.'.format(message.author.display_name))
        return

    msg = message.content.split(None, 1)[1].replace('@everyone', '@\u2020everyone').replace('@here', '@\u2020here')

    if mention_syntax.search(msg):
        await client.send_message(message.channel, '{}: To prevent abuse, you are not allowed to provide mentions in your away messages.'.format(message.author.display_name))
        return
...
async def awayhelper(message, client):
    ...
    if message.author.id in msgs and not msgs[message.author.id]['grossfix']:
        ...
        if message.channel.is_private:
            await client.send_message(message.author, '{}: Your away message has been automatically disabled.'.format(message.author.display_name))
        else:
            await client.send_message(message.author, '{}: Your away message has been automatically disabled because you sent a message in #{}'.format(message.author.display_name, message.channel.name))
    ...
    if message.mentions:
        ...
        for x in respond:
            if not msgs[x.id]['cooldown']:
                await client.send_message(message.channel, '{}: {} is away. This is their set message:\n\n{}\n\n*This is an automated response. You can set one up yourself with `.away`.*'.format(message.author.display_name, x.display_name, msgs[x.id]['msg']))
